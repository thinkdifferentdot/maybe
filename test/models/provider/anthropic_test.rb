...
class Provider::AnthropicTest < ActiveSupport::TestCase
  ...
  setup do
    @subject = @anthropic = Provider::Anthropic.new(ENV.fetch("ANTHROPIC_API_KEY", "test-token"))
  ...
  test "basic chat response" do
    VCR.use_cassette("anthropic/chat/basic_response") do
      ...
      unless response.success?
        puts "ERROR: #{response.error.class} - #{response.error.message}" if response.error
        puts "ERROR MESSAGE: #{response.error.inspect}"
      end
