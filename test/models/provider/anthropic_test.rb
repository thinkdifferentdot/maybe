...
class Provider::AnthropicTest < ActiveSupport::TestCase
  ...
  setup do
    @api_key = ENV.fetch("ANTHROPIC_API_KEY", "test-token")
    @subject_model = Provider::Anthropic::DEFAULT_MODEL # claude-sonnet-4-5-20250929
  end

  # Helper method to create a fresh provider instance
  # Used in VCR tests to ensure ANTHROPIC_BASE_URL is cleared during ClimateControl block
  def create_provider
    Provider::Anthropic.new(@api_key)
  end

  test "provider_name returns Anthropic" do
    subject = create_provider
    assert_equal "Anthropic", subject.provider_name
  end

  test "supports_model? returns true for claude- models" do
    subject = create_provider
    assert subject.supports_model?("claude-sonnet-4-5-20250929")
    assert subject.supports_model?("claude-opus-4-5")
    assert subject.supports_model?("claude-3-haiku")
  end

  test "supports_model? returns false for non-claude models" do
    subject = create_provider
    assert_not subject.supports_model?("gpt-4")
    assert_not subject.supports_model?("llama-3")
  end

  test "supported_models_description returns model prefix description" do
    subject = create_provider
    expected = "models starting with: claude-"
    assert_equal expected, subject.supported_models_description
  end

  # Task 4: auto_detect_merchants test with VCR
  test "auto detects merchants" do
    # Clear ANTHROPIC_BASE_URL to ensure VCR cassettes match (they were recorded against api.anthropic.com)
  ...
  # Task 5: basic chat_response test with VCR
  ...
  # Task 6: chat with function calls test with VCR
  ...
  # Task 7: Anthropic error handling test
  test "anthropic errors are automatically raised" do
    ClimateControl.modify ANTHROPIC_BASE_URL: nil do
      subject = create_provider
      VCR.use_cassette("anthropic/chat/error") do
        response = subject.chat_response("Test", model: "invalid-model-that-will-trigger-api-error")
  ...
  # Task 8: effective_model class method test
  ...
  test "effective_model returns DEFAULT_MODEL when ENV is empty string" do
